...
采用 **Iconfont 手动导出 SVG → base64 Data URI → WXML `<image>` 标签** 的方案，不使用第三方图标库。

### Why

微信小程序不支持 `<svg>` 标签和外部 web 字体（`@font-face`），业界主流方案是将 SVG 转为 base64 Data URI 嵌入 `<image>` 标签。项目图标需求量有限（预计 20 个以内），手动管理更轻量，零运行时依赖。
...
1. 在 [iconfont.cn](https://www.iconfont.cn) 上管理项目图标集
2. 按需下载单个图标的 SVG 文件
...
- **TabBar 图标必须用本地 PNG**。需要 TabBar 图标时，额外导出 PNG 或通过脚本生成。
- **SVG 内联 `<style>` 会被忽略**，图标样式必须用 `fill` / `stroke` 属性。
- **Unicode 编码**：原生 `btoa` 不支持中文/特殊字符，必须用 `js-base64` 处理。
- 单个图标 Data URI 通常 < 5KB，无渲染性能风险。

### 被拒绝的替代方案

| 方案 | 拒绝原因 |
|---|---|
| `lucide-react-taro` | 项目非 Taro 技术栈 |
| `@mp-svg-icons/wechat` | 图标量小，引入库不如手写 |
| `weapp-svg-icon` | CSS mask 方案仅支持单色，且依赖重 |
| TDesign `t-icon` | 项目未使用 TDesign 组件库 |
