...
- 新增云函数 `content-security`，封装微信开放能力 `security.msgSecCheck`（文本安全）和 `security.urlSecCheck`（URL 检测）。云函数在云开发环境中调用，因为这两个 API 需要云调用权限。
- 新增客户端工具模块 `lib/content-security.ts`，暴露 `checkText(text: string): Promise<{pass: boolean; reason?: string}>` 和 `checkImage(tempFilePath: string): Promise<{pass: boolean; reason?: string}>`。
  - `checkText`：构造 `{content: text, version: 2, scene: 2}` 调用云函数，检测文本中是否包含敏感字和 URL。`urlSecCheck` 和 `msgSecCheck` 均需执行，任一不通过即返回不通过。
  - `checkImage`：小程序端直接调用 `wx.security.imgSecCheck({media: [{tempFilePath}]})`，因为图片检测无需云调用权限。
...
- 新增 `cloudfunctions/content-security/index.js`，暴露 `checkText` 方法，内部依次调用 `cloud.openapi.security.msgSecCheck` 和 `urlSecCheck`，返回 `{pass, reason}`。
...
- 图片检测 `imgSecCheck` 要求图片大小不超过 1MB 且格式为 jpg/png/bmp/gif。菜品图片上传前可能需要在前端做一次尺寸/格式校验。
